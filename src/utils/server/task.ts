  ...
  create: async (
    ...
    task.promise
      .then((result) => {
        ...
        console.log(
          `task "${id}" finished at ${current}, time taken : ${current.getTime() - startTime}ms`,
        );
      })
      .catch((error) => {
        task.status = 'failed';
        ...
        const current = new Date();
        console.error(
          `task "${id}" failed at ${current}, time taken : ${current.getTime() - startTime}ms`,
          error,
        );
      })
      .finally(() => {
  ...
  wait: async (id: string, timeout: number = 30000): Promise<any> => {
    ...
    return Promise.race([
      ...
      new Promise<any>((_, reject) => {
        setTimeout(
          () => reject(new Error(`task wait "${id}" out of time`)),
          timeout,
        );
      }),
    ]);
