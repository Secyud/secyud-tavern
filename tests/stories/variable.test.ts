...
import { RealmMessage } from '@/stories/realms';

function makeHistory(): RealmMessage {
  return {
    content: '原内容',
    variables: [{ op: 'add', path: '/a', value: {} }],
    properties: {},
  };
}
...
describe('extractVariableChanges', () => {
  ...
  it('单个对象块被解析为变更项', () => {
    ...
    realms.fill(
      history,
      '回复。<variable_changes>{"op":"replace","path":"/player/hp","value":80}</variable_changes>',
    );
    expect(history.variables).toEqual([
  ...
  it('数组块中的多个合法项全部被解析', () => {
    const history = makeHistory();
    const text =
      '战斗继续。<variable_changes>[{"op":"replace","path":"/player/hp","value":80},{"op":"replace","path":"/player/mp","value":50},{"op":"remove","path":"/player/status"}]</variable_changes>';
    realms.fill(history, text);
    expect(history.variables).toEqual([
      { op: 'replace', path: '/player/hp', value: 80 },
  ...
  it('数组中混入的非法项会被过滤掉', () => {
    const history = makeHistory();
    const text =
      '<variable_changes>[{"op":"replace","path":"/a","value":1},"hello",{"op":"remove"},123,{"path":"/b"}]</variable_changes>';
    realms.fill(history, text);
    expect(history.variables).toEqual([
  ...
  it('缺少 value 的项只要 op 和 path 合法也会保留', () => {
    ...
    realms.fill(
      history,
      '<variable_changes>{"op":"remove","path":"/player/buff"}</variable_changes>',
    );
  ...
  it('多个块会合并解析, 顺序保持', () => {
    const history = makeHistory();
    const text =
      '你受伤了。<variable_changes>{"op":"replace","path":"/hp","value":80}</variable_changes>随后获得增益。<variable_changes>{"op":"add","path":"/buffs/shield", "value": "value"}</variable_changes>';
    realms.fill(history, text);
    expect(history.variables).toEqual([
  ...
  it('块内 JSON 非法时跳过该块并移除标签, 不影响其他块', () => {
    const history = makeHistory();
    const text =
      '开头<variable_changes>{not valid json}</variable_changes>中间<variable_changes>{"op":"replace","path":"/a","value":1}</variable_changes>结尾';
    realms.fill(history, text);
    expect(history.variables).toEqual([
      { op: 'replace', path: '/a', value: 1 },
    ]);
  ...
  it('块前后文本保留, 输入首尾空白在替换前被去除', () => {
    const history = makeHistory();
    const text =
      '  她缓缓抬起头。  <variable_changes>{"op":"replace","path":"/mood","value":"平静"}</variable_changes>  ';
    realms.fill(history, text);
    expect(history.variables).toEqual([
      { op: 'replace', path: '/mood', value: '平静' },
    ]);
